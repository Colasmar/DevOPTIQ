    ...
    for shape in shapes:
        ...
        # Identifier les connexions pour les données
            ...
            if connections["from_id"] in activity_data:  # Sortante
                if layer_value == "10":
                    activity_data[connections["from_id"]]["declenchante_out"].append(shape.text.strip())
                elif layer_value == "9":
                    activity_data[connections["from_id"]]["nourrissante_out"].append(shape.text.strip())
            if connections["to_id"] in activity_data:  # Entrante
                if layer_value == "10":
                    activity_data[connections["to_id"]]["declenchante_in"].append(shape.text.strip())
                elif layer_value == "9":
                    activity_data[connections["to_id"]]["nourrissante_in"].append(shape.text.strip())
...
def test_activities_with_data():
    """
    Test pour analyser les connexions entre activités et données.
    """
...
if __name__ == "__main__":
    test_activities_with_data()
    print("DEBUG: Fin du script test_activities_with_data.py")
