...
document.addEventListener('DOMContentLoaded', () => {
    const managerDiv = document.getElementById('manager-info');
    currentManagerId = managerDiv.dataset.managerId;

    loadCollaborators(currentManagerId);

    document.getElementById('collaborator-select').addEventListener('change', () => {
        selectedUserId = document.getElementById('collaborator-select').value;
        if (selectedUserId) loadUserRoles(selectedUserId);
    });

    document.getElementById('user-roles').addEventListener('change', handleRolesChange);

    const saveBtn = document.getElementById('save-competencies-button');
    saveBtn?.addEventListener('click', saveAllEvaluations);
});
...
function loadUserRoles(userId) {
    fetch(`/competences/get_user_roles/${userId}`)
        ...
        .then(data => {
            ...
            select.innerHTML = '';
            allRolesCache = {}; // Réinitialise le cache

            data.roles.forEach(role => {
                const opt = document.createElement('option');
                opt.value = role.id;
                opt.textContent = role.name;
                allRolesCache[role.id] = role.name;
                select.appendChild(opt);
            });
        });
}
...
function handleRolesChange() {
    const selected = Array.from(document.getElementById('user-roles').selectedOptions).map(o => o.value);
    const container = document.getElementById('roles-section');
    container.innerHTML = ''; // Reset

    selected.forEach(roleId => createRoleSection(roleId));
}

function createRoleSection(roleId) {
    const section = document.createElement('section');
    section.className = 'role-section';

    const title = document.createElement('h3');
    title.textContent = `Rôle : ${allRolesCache[roleId] || 'Inconnu'}`;
    ...
    synthesisDiv.className = 'synthesis-container';
    section.appendChild(synthesisDiv);

    document.getElementById('roles-section').appendChild(section);

    loadRoleActivities(roleId, activitiesDiv);
    loadCompetencySynthesis(roleId, synthesisDiv);
}

function loadRoleActivities(roleId, container) {
    fetch(`/competences/get_activities_for_role/${roleId}`)
        .then(res => res.json())
        .then(activities => {
            activities.forEach(act => {
                ...
                const perfPlaceholder = document.createElement('div');
                perfPlaceholder.textContent = 'Performances (à implémenter)';
                actDiv.appendChild(perfPlaceholder);

                const button = document.createElement('button');
                button.textContent = 'Afficher le suivi des compétences';
                button.addEventListener('click', () => loadActivityKnowledge(act.id, actDiv));
