...
def fix_shape_id_index():
    ...
    try:
        ...
        try:
            ...
            db.session.rollback()
            result["actions"].append(f"Suppression ix_activities_shape_id: {str(e)[:100]}")
        
        # Vérifier si l'index correct existe déjà (PostgreSQL)
        try:
        ...
        if not existing:
            ...
            try:
                # PostgreSQL - index partiel avec WHERE
                ...
                db.session.rollback()
                # Essayer sans le WHERE (si le WHERE pose problème)
                try:
                    db.session.execute(text("""
                        CREATE UNIQUE INDEX ix_activities_entity_shape 
                        ON activities(entity_id, shape_id)
                    """))
                    db.session.commit()
                    result["actions"].append("Créé: ix_activities_entity_shape (sans WHERE)")
                except Exception as e2:
                    result["errors"].append(f"Création index: {str(e2)[:100]}")
        else:
